%% This fonction return the mean square_error of data inputs using Kernel_ridge_regression_sqrexp
...
function[err] = Kernel_ridge_regression_sqrexp_parameters (X , Y , lambda , sigma , mu , nv , p)

k=@(x,y) (x*y').^2 + mu*exp(-(x*ones(1,size(y,1))-ones(size(x,1),1)*y').^2/(2*sigma^2));
